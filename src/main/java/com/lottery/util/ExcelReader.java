...
public class ExcelReader {
    ...
    public String[] readExcelTitle(InputStream is) {
        ...
        for (int i = 0; i < colNum; i++) {
        	String t = getCellFormatValue(row.getCell(i));
            title[i] = t.substring(t.indexOf("(")+1, t.indexOf(")"));
        }
        return title;
    }
        ...
        if(rowNum>0){
             ...
             for (int i = 1; i <= rowNum; i++) {
                 ...
                 while (j < colNum) {
                     ...
                     String setMethodName = "set"+(fields[j].substring(0, 1)).toUpperCase()+(fields[j].substring(1)).toLowerCase();
                     Method setMethod = User.class.getMethod(setMethodName, new Class[]{String.class});
...
//
//            // 对读取Excel表格内容测试
//            InputStream is2 = new FileInputStream("c:\\test.xls");
